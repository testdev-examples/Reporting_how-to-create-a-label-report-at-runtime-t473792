...
Namespace dxWinFormsSample
	Public Class CustomLabelReportBuilder
		...
		Private Function BuildLabelReport(ByVal model As CustomLabelReportModel) As XtraReport
			...
			Dim top As Integer = CInt(Math.Truncate(XRConvert.Convert(model.TopMargin, labelDpi, report.Dpi)))
			Dim left As Integer = CInt(Math.Truncate(XRConvert.Convert(model.LeftMargin, labelDpi, report.Dpi)))
			Dim right As Integer = CInt(Math.Truncate(XRConvert.Convert(model.RightMargin, labelDpi, report.Dpi)))
			Dim bottom As Integer = CInt(Math.Truncate(XRConvert.Convert(model.BottomMargin, labelDpi, report.Dpi)))
